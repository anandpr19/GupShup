...
const SideBar = ({ selectedUser, setSelectedUser }) => {
    ...
    return (
        <div className={`bg-[#8185B2] h-full p-5 rounded-r-xl overflow-y-auto text-white ${selectedUser ? "max-md:hidden" : ""}`}>
            {/* Top Section */}
            <div className="pb-5">
            ...
            <div>
                ...
                <div className='flex flex-col'>
                    {userDummyData.map((user,index)=>(
                        <div onClick={()=>{selectedUser(user)}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id ===user.id && 'bg-[#282142]/50' }`}>
